import sqlite3 # importamos nuestro gestor de BBDD

# validamos desde la BBDD
def consultar_usuario(correo, password): # enviamos parametros
    db=sqlite3.connect("mensajes.s3db") # nos conectamos a la BBDD
    db.row_factory=sqlite3.Row # envia las cabeceras para tenerlas como variables
    cursor=db.cursor() # Apuntador para realizar las consultas, recibe datos de la BBDD
    consulta="select *from usuarios where correo='"+correo+"' and password='"+password+"' and estado='1'" # cadena que me permite hacer la consulta
    cursor.execute(consulta) # el apuntador tiene la BBDD, y ejecutamos la consulta
    return cursor.fetchall() # retornamos todos los registro que encuentre con la funcion fetchall
...
def lista_destinatarios(correo):
    ...
    cursor=db.cursor()
    consulta="select *from usuarios where  estado='1' and correo<>'"+correo+"'" # la diferencia hara que no muestre nuestro usuario en la lista
...
def regisUsuario(nombe,correo, password,codigo):
    try:
        ...
        cursor.execute(consulta)
        db.commit() # Confirma la insercion a la BBDD
        return "1"
    except :
        return "0"
